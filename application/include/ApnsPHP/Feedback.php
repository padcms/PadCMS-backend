		...
		while (!feof($this->_hSocket)) {
			...
			if ($nBufferLen >= $nFeedbackTupleLen) {
				...
				for ($i = 0; $i < $nFeedbackTuples; $i++) {
					...
					$this->getLogger()->info(sprintf("New feedback tuple: timestamp=%d (%s), tokenLength=%d, deviceToken=%s.",
			...
			if ($nChangedStreams === false) {
				$this->getLogger()->warn('Unable to wait for a stream availability.');
				break;
			}
		}
