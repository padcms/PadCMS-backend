...
class ApnsPHP_Push extends ApnsPHP_Abstract
{
	...
	public function send()
	{
		...
		while (($nMessages = count($this->_aMessageQueue)) > 0) {
			$this->getLogger()->info("Sending messages queue, run #{$nRun}: $nMessages message(s) left in queue.");
			...
			foreach($this->_aMessageQueue as $k => &$aMessage) {
				...
				if (!empty($aMessage['ERRORS'])) {
					foreach($aMessage['ERRORS'] as $aError) {
						if ($aError['statusCode'] == 0) {
							$this->getLogger()->info("Message ID {$k} {$sCustomIdentifier} has no error ({$aError['statusCode']}), removing from queue...");
							$this->_removeMessageFromQueue($k);
							continue 2;
						} else if ($aError['statusCode'] > 1 && $aError['statusCode'] <= 8) {
							$this->getLogger()->warn("Message ID {$k} {$sCustomIdentifier} has an unrecoverable error ({$aError['statusCode']}), removing from queue without retrying...");
				...
				$this->getLogger()->info("Sending message ID {$k} {$sCustomIdentifier} (" . ($nErrors + 1) . "/{$this->_nSendRetryTimes}): {$nLen} bytes.");

				$aErrorMessage = null;
				if ($nLen !== ($nWritten = (int) AM_Tools_Standard::getInstance()->fwrite($this->_hSocket, $aMessage['BINARY_NOTIFICATION'], $nLen))) {
					$aErrorMessage = array(
						'identifier' => $k,
			...
			if (!$bError) {
				...
				$null = NULL;
				$nChangedStreams = AM_Tools_Standard::getInstance()->stream_select($read, $null, $null, 0, $this->_nSocketSelectTimeout);
				if ($nChangedStreams === false) {
					$this->getLogger()->err('Unable to wait for a stream availability.');
					break;
	...
	protected function _readErrorMessage()
	{
        ...
        /**
         * Get error message by code in error-response packet
         * @param int $statusCode
         * @return string
         */
        protected function _getErrorMessageByCode($statusCode)
        {
            if (!array_key_exists($statusCode, $this->_aErrorResponseMessages)) {
                return 'Unknown error';
            }

            return $this->_aErrorResponseMessages[$statusCode];
        }
	...
	protected function _updateQueue($aErrorMessage = null)
	{
		...
		$this->getLogger()->err('Unable to send message ID ' .
